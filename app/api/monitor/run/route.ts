...
async function checkWebsiteHealth(): Promise<MonitorCheck[]> {
  ...
  checks.push(await checkUrlWithRetry("/cart", "עגלה ורכישה"));
  checks.push(await checkUrlWithRetry("/api/orders?account=monitor-check%40example.invalid", "Orders API", "Customer area"));
  checks.push(await checkUrlWithRetry("/", "דף הבית"));
  checks.push(await checkUrlWithRetry("/products?page=1", "עמוד כל הספרים"));
  checks.push(await checkUrlWithRetry("/wishlist", "עמוד מועדפים"));
  checks.push(await checkUrlWithRetry("/account", "אזור אישי"));
  checks.push(await checkUrlWithRetry("/cart", "עמוד העגלה"));
...
async function resolveHealthyApprovalRequests(checks: MonitorCheck[]) {
  ...
  if (healthyKeys.includes("url:/api/orders?account=monitor-check%40example.invalid")) {
    ...
    legacyHealthyFingerprints.add("internal-monitor:url:/api/orders?account=0500000000:error");
    legacyHealthyFingerprints.add("internal-monitor:url:/api/orders?account=0500000000:warning");
  }
